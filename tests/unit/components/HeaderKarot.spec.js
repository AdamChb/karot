...
describe('HeaderKarot.vue', () => {
  ...
  it('renders the header with navigation links', () => {
    ...
    expect(wrapper.findAll('nav a')).toHaveLength(4) // 4 links in total
    expect(wrapper.find('nav a:nth-child(1)').text()).toBe('Create meals')
    expect(wrapper.find('nav a:nth-child(2)').text()).toBe('Recipes')
    expect(wrapper.find('nav a:nth-child(3)').text()).toBe('My meals')
    expect(wrapper.find('nav a:nth-child(4)').text()).toBe('Log In')
  })
  ...
  it('toggles the mobile menu when clicked', async () => {
    ...
    expect(wrapper.find('#header_nav_burger').element.style.display).toBe('flex')

    // Click again to toggle the menu off
    await wrapper.find('#menu_button').trigger('click')

    // Check that the mobile menu is hidden
    expect(wrapper.find('#header_nav_burger').element.style.display).toBe('none')
  })
